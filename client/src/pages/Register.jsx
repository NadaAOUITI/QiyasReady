...
export function Register() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      setErr(e.data?.error || e.message || "تعذر إنشاء الحساب");
    } finally {
      setBusy(false);
    }
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-slate-100 to-slate-50 px-4 page-enter">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-slate-200 p-8">
        <div className="text-center mb-6">
          <h1 className="text-2xl font-bold text-brand">حساب جديد</h1>
          <p className="text-slate-500 text-sm mt-1" dir="ltr" lang="en">
            Student account · free demo
          </p>
        </div>
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">كلمة المرور (6+)</label>
            <input
              type="password"
              className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-gold/60 focus:border-brand outline-none"
              value={password}
          ...
          <button
            type="submit"
            disabled={busy}
            className="w-full py-2.5 rounded-lg bg-gold text-brand font-semibold hover:opacity-95 disabled:opacity-60"
          >
